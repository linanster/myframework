...
def background_thread():
    ...
    while True:
        socketio.sleep(10)
        count += 1
        socketio.emit('mybackground', {'data': 'Server generated event', 'count': count}, namespace='/test')
...
def socket_connect():
    ...
    # 1. echo 'Connected'
    msg = 'Connected!'
    socketio.emit('mymsg', msg, namespace='/test')
    # 2. background thread1
    global thread1
    with thread_lock1:
        if thread1 is None:
            socketio.emit('mybackground', {'data': 'Start background thread', 'count': 0}, namespace='/test')
    ...
    with thread_lock2:
        if thread2 is None:
            socketio.emit('mylog', 'Start watch log', namespace='/test')
